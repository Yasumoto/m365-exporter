...
func TestCollector_scrapePerPolicyCompliance(t *testing.T) {
	...
	collector := NewCollector(logger, tenantID, msGraphClient, httpClient.GetHTTPClient(), Settings{
		PerPolicyCompliance: true,
	})

	// TODO: Go 1.24: Change to t.Context()
	metrics, err := collector.scrapePerPolicyCompliance(context.Background())
	require.NoError(t, err)

	// Per-policy compliance metrics might be empty if no policies exist
	allMetrics, err := testutil.MetricsToText(t, metrics)
	require.NoError(t, err)

	// If compliance policies exist, we should see the metrics
	if len(metrics) > 0 {
		assert.Contains(t, allMetrics, "m365_intune_device_policy_compliance")
	}
}
